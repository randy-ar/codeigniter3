...
class m_barang extends CI_Model
{
    ...
    function tambah()
    {
        ...
        $data = [
            ...
            'harga' => $harga,
            'stok' => $stok,
            'foto' => null  // Set null jika tidak ada foto yang diunggah
        ];
        ...
        if ($_FILES['foto']['name']) {
            if ($this->upload->do_upload('foto')) {
                ...
                // Cek tipe file dan compress gambar
                ...
                if ($file_type == 'image/jpeg' || $file_type == 'image/jpg') {
                    ...
                    imagepng($image, $config['upload_path'] . '/' . $gbr['file_name'], 5);
                } elseif ($file_type == 'image/gif') {
                    $image = imagecreatefromgif($source_url);
                    imagegif($image, $config['upload_path'] . '/' . $gbr['file_name']);
                }

                //Update data dengan nama file foto yang berhasil diunggah
    ...
    function selectBarang($idBarang)
    {
        $this->db->select('*');
        $this->db->from('barang');
